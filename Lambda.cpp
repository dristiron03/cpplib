...
void Example2() {
	...
	int ar[] = {1, 5, 65, 23, -3, -7, -43, -2, -54, 45, 2};

	int n = sizeof(ar) / sizeof(int);
	sort(ar, ar + n, [&](int x, int y) {
		return abs(x) < abs(y);
	});
...
void Example3() {
	// compiler binds a lambda expression to its captured
	// variables when the expression is declared instead
